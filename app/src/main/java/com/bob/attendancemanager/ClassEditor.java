...
public class ClassEditor extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Check if the intent has extra "Class ID"
        if (getIntent().hasExtra("Class ID")) {
            ...
            // If no extra "Class ID", get the next class ID from the database
            classID = database.getNextClassID();
        }

        // Get students for the current class ID
        ...
        create.setOnClickListener(v -> {
            ...
            Class c = new Class(classID, class_name.getText().toString());
            // Add the class to the database
            database.addClass(c);
            // Disable class name editing
            class_name.setEnabled(false);
            // Hide the create button
            create.setVisibility(View.GONE);
            // Show the add students and delete buttons
